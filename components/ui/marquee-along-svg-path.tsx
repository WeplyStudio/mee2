...
const MarqueeAlongSvgPath = ({
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      const rawScale = responsiveFit === "width" ? scaleX : Math.min(scaleX, scaleY)
      ...
      // Center the marquee container within the wrapper, clamping offsetY to prevent top clipping
      const offsetX = (wrapperWidth - scaledWidth) / 2
      const naturalOffsetY = (wrapperHeight - scaledHeight) / 2
      const offsetY = responsiveFit === "width" ? Math.max(naturalOffsetY, 8) : naturalOffsetY
      ...
      // Expose scale to CSS for item inverse-scaling if desired
      marqueeContainer.style.setProperty("--marquee-scale", scale.toString())
  ...
  }, [responsive, responsiveFit, minScale, viewBox])
  ...
  // Scroll tracking
  // If scrollContainer is null or undefined, useScroll tracks window scroll
  const { scrollY } = useScroll(
    scrollContainer ? { container: scrollContainer as RefObject<HTMLDivElement | null> } : {}
  )
  ...
  // Transform scroll velocity into a factor that smoothly accelerates marquee movement
